...
# ── Set review status based on snapshot.next ──────────────────────────
        ...
        if interrupted:
            ...
            # --- FIX START: Sync summary from LangGraph to DB ---
            # Even though we are pausing, we want the Chat Service to see 
            # the summary generated by summary_node.
            if full_state and "summary" in full_state:
                review.summary = full_state["summary"]
                logger.info("[trigger_review] Syncing summary to DB for review_id=%d", review.id)
            # --- FIX END ---
